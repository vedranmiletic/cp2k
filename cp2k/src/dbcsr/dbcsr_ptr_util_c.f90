...
!   ---------------------------------------------------------------------------
    ...
    IF (.NOT.ASSOCIATED(array)) THEN
       ...
       CALL mem_alloc_c (array, ub, mem_type=mem_type, error=error)
    ...
    IF(.NOT.docopy) THEN
       CALL mem_dealloc_c (array, mem_type=mem_type, error=error)
    ENDIF
    !
    ...
    CALL dbcsr_assert (lb_new, "EQ", 1, &
         dbcsr_fatal_level, dbcsr_unimplemented_error_nr, routineN,&
         "Arrays must start at 1", __LINE__, error=error)
    CALL mem_alloc_c (newarray, ub_new-lb_new+1, mem_type, error=error)
    !
...
! *****************************************************************************
  SUBROUTINE mem_alloc_c (mem, n, mem_type, error)
